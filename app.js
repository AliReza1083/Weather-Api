...
//* page 2
let api2 = 'https://api.openweathermap.org/data/2.5/weather?q=tehran&appid=f88e5d58f9da413209e8ccd1c634eeb1';

app.get('/weatherapi/datas', (req, res) => {
    fetch(api2)
        .then((val) => {
            return val.json()
        })
        .then((data) => {
            // const cDegree = Math.floor(data.main.temp - 273.15);
            if (data.cod == '404') {
                res.render('apiData', {data, randImg})
            } else {
                res.render('apiData', {data, randImg})
            }
        })
...
app.post('/weatherapi/datas', (req, res) => {
    const {search} = req.body;
    api2 = `https://api.openweathermap.org/data/2.5/weather?q=${search}&appid=f88e5d58f9da413209e8ccd1c634eeb1`;
    res.redirect('/weatherapi/datas')
})
